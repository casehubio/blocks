...
public final class TtsPipeline implements TextToSpeechService, AutoCloseable {
    ...
    private static TtsPipeline buildCosyVoice3(Path modelDir, CosyVoice3Manifest manifest) {
        ...
        var speechEmbedding       = ort.createSession(modelDir.resolve("llm_speech_embedding_fp16.onnx"), threads, OnnxRuntimeLibrary.GRAPH_OPT_ENABLE_BASIC);
        var llmInitial            = ort.createSession(modelDir.resolve("llm_backbone_initial_fp16.onnx"), threads, OnnxRuntimeLibrary.GRAPH_OPT_ENABLE_BASIC);
        var llmDecode             = ort.createSession(modelDir.resolve("llm_backbone_decode_fp16.onnx"), threads, OnnxRuntimeLibrary.GRAPH_OPT_ENABLE_BASIC);
        var llmDecoder            = ort.createSession(modelDir.resolve("llm_decoder_fp16.onnx"), threads, OnnxRuntimeLibrary.GRAPH_OPT_ENABLE_BASIC);
        var flowTokenEmbedding    = ort.createSession(modelDir.resolve("flow_token_embedding_fp16.onnx"), threads, OnnxRuntimeLibrary.GRAPH_OPT_ENABLE_BASIC);
        var flowPreLookahead      = ort.createSession(modelDir.resolve("flow_pre_lookahead_fp16.onnx"), threads, OnnxRuntimeLibrary.GRAPH_OPT_ENABLE_BASIC);
        var flowSpeakerProjection = ort.createSession(modelDir.resolve("flow_speaker_projection_fp16.onnx"), threads, OnnxRuntimeLibrary.GRAPH_OPT_ENABLE_BASIC);
        var flowEstimator         = ort.createSession(modelDir.resolve("flow.decoder.estimator.fp16.onnx"), threads, OnnxRuntimeLibrary.GRAPH_OPT_ENABLE_BASIC);
