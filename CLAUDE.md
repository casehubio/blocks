...
| `src/main/java/io/casehub/blocks/agentic/social/emergence/` | Social emergence — `SocialNorm` (detected behavioral pattern with `NormStrength`), `NormObservation` (interaction pattern for CBR storage), `DetectedNorms` (with `established()` filter), `NormDetectionConfig`/`NormDetectionTick`, `SocialNormDetector` (`@ApplicationScoped` compositor — reads NormObservation cases from CBR, groups by behavioralPattern, computes adherence rates, classifies strength EMERGING/ESTABLISHED/DECLINING, per-tenant caching), `NormObservationSchema` (CBR serialization), `DriveAlignment` (pairwise drive profile alignment), `CollectiveGoalProposal`/`CollectiveGoalTick`/`CollectiveGoalConfig` |
